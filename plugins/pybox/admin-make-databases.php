...
function cscircles_makedb_page() {
    ...
    echo "<div class='wrap'>
        <h2>Rebuild CS Circles Databases,注意:标题规范  数字: 标题 , 标题前有个空格</h2>
    ...
    if (!array_key_exists('submitted', $_REQUEST)) {
        ...
        foreach ($pages as $page) {
            ...
            $titleMatch = preg_match('/^([0-9]+)([A-Za-z]?)\: (.*)$/', $title, $matches);
...
//        }
        ...
        $i        = -1;
        $gets     = array();
        echo "<br/><b>Total Lesson".count($lessons)."</b><br/>";
        foreach ($lessons as $lesson) {
            echo '<pre>';
            if ($currlang != $lesson['lang']) {
                $currlang = $lesson['lang'];
                $i        = 0;
            }
            ...
            $lesson['ordering'] = $index;

            if ($lesson['number'] != NULL) {
                echo '<b>Lesson:</b> ' . rowSummary($lesson); //. json_encode($l);
                if (!$GLOBALS['SKIP_DB_REBUILD'])
                    echo($wpdb->insert($lesson_table_name, $lesson) != 1 ? '<span style="color:red">insert bad</span>' : ' <span style="color:green">insert ok</span>');
            }

            global $lesson_reg_info, $pyRenderCount, $post;

            $lesson_reg_info = array(
                'index'      => $index,
                'lang'       => $currlang,
                'fullnumber' => $lesson['number'],
                'url'        => str_replace('/dev', '', get_page_link($lesson['id'])), 'id' => $lesson['id']
            );

            // render! the following line is not just cosmetic as it registers the problems.
            echo '<br>Snippet: ' . htmlspecialchars(substr(do_sweetcode(get_page($lesson['id'])->post_content), 0, 50));
