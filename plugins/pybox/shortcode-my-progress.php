...
function pyUser($options, $content) {
    ...
    if (userIsAdmin() || userIsAssistant() || $studentsCount > 0) {
        ...
        if (userIsAdmin()) {
            $preamble .= 'blank: you; "all": all; id#: user (<a href="' .
                         csCurlRouter('allusers') .
        ...
        $preamble .= "</td></tr><tr><td colspan='2' style='text-align:center'><input style='width: 25%' type='submit' value='" .
                     __t('Submit') .
                     "'/></tr></td></table></form><a href='/wp-content/plugins/pybox/page-gradesheet.php'>CSV Downloader</a></div>";
        echo $preamble;
    }
    ...
    if (!$allStudents && array_key_exists('user', $_GET) && $_GET['user'] != '') {
        ...
        echo "<div class='history-prenote'>" . sprintf(__t("Now viewing %s for "), $problem_html) . userNickOrUserLogin($uid) . '</div>';
    }
    if ($allStudents) {
