...
/**
 * User Page, User Progress,User Student
 */
...
function pyUser($options, $content) {
    ...
    // Check problem parameter in GET request object??
    $problemValueInGetRequestObject = getSoft($_GET, "problem", "");
    if ($problemValueInGetRequestObject != "" && $problemValueInGetRequestObject != "console" && !array_key_exists($problemValueInGetRequestObject, $problemsByNumber)) {
        echo sprintf(__t("Problem %s not found (at least in current language)"), $problemValueInGetRequestObject);
        return;
    }
    // If the login user is admin ,or Assistant or the one whose student count is more than 1 , show the table.
    if (userIsAdmin() || userIsAssistant() || $studentsCount > 0) {
        $preamble       = "<div class='progress-selector'>
                               <form method='get'>
                                   <table style='border:none'>
                                       <tr>
                                           <td>" . sprintf(__t("View one of your students? (you have %s)"), $studentsCount) . '</td>
                                           <td>';
                                        $options        = array();
                                        ...
                                        if (userIsAdmin()) {
                                            $preamble .= 'blank: you; "all": all; id#: user (<a href="' .csCurlRouter('allusers') .
                                                         '">list</a>) <br/><input style = "padding:0px;width:60px" type="text" name="user" value="' .
                                                         getSoft($_REQUEST, 'user', '') .
                                                         '">';
                                        } else {
                                        ...
                                        $preamble .= "</td></tr><tr><td colspan='2' style='text-align:center'>
                                                            <input style='width: 25%' type='submit' value='" .
                                                            __t('Submit') .
                                                     "'/></tr></td></table></form>
                                                            <a href='/wp-content/plugins/pybox/page-gradesheet.php'>CSV Downloader</a>
                                                            </div>";
                                        echo $preamble;
                                    }
                                    ...
                                    $allProblems = ($problemValueInGetRequestObject == "");

                                    if (!$viewingAsStudent) {
                                        if ($allProblems){
                                            $problem_html = "all problems";
                                        } else if ($problemValueInGetRequestObject == 'console'){
                                            $problem_html = "Console";
                                        } else{
                                            $problem_html
                                                = "<a href='" . $problemsByNumber[$problemValueInGetRequestObject]['url'] . "'>" .
                                                  $problemsByNumber[$problemValueInGetRequestObject]['publicname'] . "</a>";
                                        }
                                    }
                                    ...
                                    if (!$allStudents && array_key_exists('user', $_GET) && $_GET['user'] != '') {
                                        ...
                                        echo "<div class='history-prenote'>" . sprintf(__t("Now viewing %s for "), __t($problem_html)) . userNickOrUserLogin($uid) . '</div>';
                                    }
                                    if ($allStudents) {
                                        echo "<div class='history-prenote'>" . sprintf(__t("Now viewing %s for all of your students"), __t($problem_html)) . "</div>";
                                    }
                                    ...
                                    // All Student & not All Problem
                                    if ($allStudents && !$allProblems && $problemValueInGetRequestObject != "console") {
                                    ...
                                    if ($allStudents && !userIsAdmin()) {
                                        ...
                                        if (!$allProblems)
                                            $where .= $wpdb->prepare("and problem LIKE %s", $problemValueInGetRequestObject);
                                        ...
                                        $studentTable .= '<div class="history-note">'.__t("Student listing (click name to drill down)").'</div>';
                                        $studentTable .= '<table>';

                                        foreach (getStudents() as $stu) {
                                            $studentTable .= '<tr>';
                                            $studentTable .= '<td>';
                                            $studentTable .= '<a class="open-same-window" href="?user=' . $stu . '&problem=' . $problemValueInGetRequestObject . '">';
                                            $studentTable .= userNickOrUserLogin($stu);
                                            $studentTable .= '</a></td>';
                                            $studentTable .= '<td>';

                                            if ($allProblems)
                                                $studentTable .= (array_key_exists($stu, $scompleted) ? ($scompleted[$stu]->comps) : 0) .__t(" completed");
                                            else
                                            ...
                                            $studentTable .= '</td>';
                                            $studentTable .= '<td>';
                                            $studentTable .= (array_key_exists($stu, $ssubmissions) ? ($ssubmissions[$stu]->subs) : 0) . __t(" submissions");
                                            $studentTable .= '</td>';
                                            $studentTable .= '</tr>';
                                        }
                                        $studentTable .= '</table>';
                                    ...
                                    if ($allProblems || !$allStudents) {
                                        ...
                                        if (!$viewingAsStudent) {
                                            $overview .= "<div style='text-align:center'>".__t("Click on the ")."<img style='height:1em;width:1em' src='" . UFILES . "/icon.png'>".__t(" to drill down.")."</div>";
                                        }
