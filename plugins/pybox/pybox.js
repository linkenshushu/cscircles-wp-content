...
function setCompleted(name) {
    if (name != 'NULL') {
        $.ajax({
            ...
            data: {"problem":name},
            error: function() {
                alert(__t("Warning: unable to talk to server. Could not set 'completed' status for this problem."))
            }
        });
    }
}
...
function sendMessage(id, slug) {
    ...
    if (recipient==0) {
        ...
        $.ajax({
            ...
            error: function() {
                alert(__t("Unable to process 'send message' request. You might have lost your internet connection."));
            }
        });
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data == '#')
                ...
                window.location = MAILURL + '?who='+id+"&what="+slug+"&which="+data+"#m";
        }
        });
...
// three types of short answer question:
// short answer, multiple choice, scramble
...
registerClick('.pyflexClose', function (e) {
    historyClick($(e.target).closest('.pybox').find('input[name="pyId"]').val(),"");
});
