...
function pberror($errmsg) {
    ...
    return "<b>" . sprintf(__t("Internal error, details below; please <a href='%s'>contact staff</a>."), csCurlRouter('contact')) . "</b> " . __t("Timestamp:") . " " . date("y-m-d H:i:s", time()) . "<br/>" . preBox($errmsg);
}

/**
 * Link handler like  HTML A
 * @param $options
 * @param $content
 *
 * @return string
...
function pyLinkHandler($options, $content) {
    ...
    $r    = '';
    $r    .= '<a href="' . csCurlRouter('console') . "?consolecode=" . rawurlencode($code) . '" target="_blank">';
    $r    .= $content;
    $r    .= '</a>';
    return $r;
}
...
function registerPybox($id, $slug, $type, $facultative, $title, $content, $args = NULL, $hash = NULL, $graderOptions = NULL) {
    ...
    if (isMakingDatabases()) {
        ...
        $row['content'] = $content;
        echo "<br>Problem: " . rowSummary($row);
        if (!$GLOBALS['SKIP_DB_REBUILD'])
            echo($wpdb->insert($table_name, $row) != 1 ? '<span style="color:red">insert bad</span>' : ' <span style="color:green">insert ok</span>');
    } else if ($hash != NULL) {
        $lang = currLang2();
...
function pyShortHandler($options, $content) {
    ...
    if (isMakingDatabases())
        return do_short_and_sweetcode($content); // faster db generation with accurate count
...
/**
 * Multiple Choice Exercise-选择题练习(多选)
 * @param $options
 * @param $content
 *
 * @return string
 ...
 * @return string
 */
function button($name, $atts) {
    $res = "<td><input type='button' name='$name' ";
...
function pyBoxHandler($options, $content) {
    ...
    if ($options == FALSE)
        $options = array();                              // wordpress does a weird thing where valueless
    ...
    $cosmeticOptions = array(
        'defaultcode', 'autocommentline', 'console',
    ...
    registerPybox($id, $slug, $scramble ? "scramble" : "code", $facultative,
                  getSoft($options, 'title', NULL), $content,
                  $shortcodeOptions, $hash, $optionsJson);
    ...
    if (!$disablericheditor) {
        ...
        if ($showEditorToggle || $richreadonly)
            $actions['CMtoggle'] = array(
                'value' => __t('Rich editor'),
