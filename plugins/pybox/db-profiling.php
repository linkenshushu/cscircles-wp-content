...
echo dbFlexigrid(
    function ($limit, $sortname, $sortorder) {
        ...
        $count = $wpdb->get_var("SELECT count(distinct activity, substring(start, 1, $L))
                FROM " . $wpdb->prefix . "pb_profiling
                $where");

        $query = "SELECT substring(start, 1, $L) as period, count(1), sum(duration), activity
                FROM " . $wpdb->prefix . "pb_profiling
                $where
